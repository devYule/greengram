...
public class OpenApiService {
    ...
    public List<ApartmentTransactionDetailVo> getApartmentTransactionList(ApartmentTransactionDto dto) throws JsonProcessingException {


        DefaultUriBuilderFactory factory = new DefaultUriBuilderFactory(openApiProperties.getBaseUrl());
        factory.setEncodingMode(DefaultUriBuilderFactory.EncodingMode.VALUES_ONLY);

        WebClient webClient = WebClient.builder()
                .filters(exchangeFilterFunctions -> exchangeFilterFunctions.add(logRequest()))
                .uriBuilderFactory(factory)
                .baseUrl(openApiProperties.getBaseUrl())
                .build();

        String data = webClient.get().uri(uriBuilder -> {
                            UriBuilder ub = uriBuilder.path(openApiProperties.getDataUrl())
                                    .queryParam("DEAL_YMD", dto.getDealYm())
                                    .queryParam("LAWD_CD", dto.getLawdCd())
                                    .queryParam("serviceKey", openApiProperties.getServiceKey());
                            if (dto.getPageNo() > 0) {
        ...
        List<ApartmentTransactionDetailVo> result = om.convertValue(jsonNode.path("body").path("items").path("item"),
                new TypeReference<>() {
                });
        return result;
    }
